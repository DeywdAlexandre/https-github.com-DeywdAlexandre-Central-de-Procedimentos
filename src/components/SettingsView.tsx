...
export const SettingsView: React.FC<SettingsViewProps> = ({ onRefresh, globalSearch = '' }) => {
  ...
  // Preferência de WhatsApp
  const [whatsappDesktopPreferred, setWhatsappDesktopPreferred] = useState<boolean>(() => {
    return typeof localStorage !== 'undefined' && localStorage.getItem('whatsapp_desktop_preferred') === 'true';
  });
              ...
              <div className="bg-white p-5 rounded-xl border border-slate-200/90 shadow-2xs">
                ...
                >
                  ...
                  <div className="sm:col-span-4 flex justify-end pt-2">
                    <button
                      type="submit"
                      className="btn-3d-primary px-4 py-2 text-xs rounded-lg shadow-sm"
                    >
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        {u.email !== 'deywd12@gmail.com' && (
                          <button
                            ...
                            className={
                              u.active
                                ? 'btn-3d-danger px-3 py-1 text-xs rounded-md'
                                : 'btn-3d-emerald px-3 py-1 text-xs rounded-md'
                            }
                          >
      ...
      {activeTab === 'parametros' && (
        <div className="bg-white p-6 rounded-xl border border-slate-200/90 shadow-2xs space-y-4 text-xs">
          ...
          <div className="space-y-3 text-slate-600 leading-relaxed">
            ...
            <div className="p-4 bg-slate-50 rounded-xl border border-slate-200 text-slate-800 space-y-3">
              <div>
                <strong className="block font-bold text-slate-900 mb-0.5">
                  4. Preferência de Envio do WhatsApp no Computador:
                </strong>
                <p className="text-slate-500 text-[11px]">
                  No celular, o sistema abre automaticamente o aplicativo oficial do WhatsApp instalado. No computador, você pode escolher se prefere usar o WhatsApp Web no navegador ou o aplicativo para Windows.
                </p>
              </div>

              <div className="flex flex-wrap items-center gap-3 pt-1">
                <label className="flex items-center gap-2 p-2 bg-white rounded-lg border border-slate-200 cursor-pointer text-xs font-semibold hover:border-slate-300">
                  <input
                    type="radio"
                    name="whatsapp_pref"
                    value="web"
                    checked={!whatsappDesktopPreferred}
                    onChange={() => {
                      setWhatsappDesktopPreferred(false);
                      localStorage.setItem('whatsapp_desktop_preferred', 'false');
                    }}
                    className="text-blue-600 focus:ring-blue-500"
                  />
                  <span>WhatsApp Web no Navegador (Reutiliza a mesma aba)</span>
                </label>
                ...
                <label className="flex items-center gap-2 p-2 bg-white rounded-lg border border-slate-200 cursor-pointer text-xs font-semibold hover:border-slate-300">
                  <input
                    ...
                    checked={whatsappDesktopPreferred}
                    onChange={() => {
                      setWhatsappDesktopPreferred(true);
                      localStorage.setItem('whatsapp_desktop_preferred', 'true');
                    }}
                    className="text-blue-600 focus:ring-blue-500"
                  />
                  <span>Aplicativo WhatsApp Desktop do Windows (Não abre abas no navegador)</span>
                </label>
              </div>
            </div>
          </div>
